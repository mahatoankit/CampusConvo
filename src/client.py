...
def record_audio(output_file=TEMP_QUERY_FILE):
    ...
    try:
        ...
        if len(frames) < 10:
            print(
                f"❌ Recording too short ({len(frames)} frames) - please speak for at least 1 second"
            )
            return False
        ...
        duration = len(frames) * RECORDING_CHUNK / RECORDING_RATE
        print(f"✓ Recording complete ({len(frames)} frames, {duration:.1f}s)")
        return True
    ...
    while True:
        try:
            ...
            if not query_text:
                ...
                print("Please try speaking more clearly or closer to the microphone...")
                continue
